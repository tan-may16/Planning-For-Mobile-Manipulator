...
void A_star_2D(int dX[],int dY[], int Gx, int Gy, double*map, int collision_thresh, int x_size, int y_size, int Rx, int Ry, unordered_map <int , state*> &OPEN_STATES, vector<state*> &path, bool policy=true, int weight = 1, bool backtrack = true, bool check_validity= true)
{
    ...
    while (!OPEN.empty())
    {
        ...
        for(int dir = 0; dir < NUMOFDIRS; dir++)
        {
            ...
            if (check_validity)
            {
                if (successor->x >= 0 && successor->x < x_size && successor->y >= 0 && successor->y < y_size && Is_valid(map, successor, 4, 4, x_size, y_size))
                {
...
void A_star_3D(int dTheta_3D[], int Gx, int Gy, int Gtheta, int Rx, int Ry, int Rtheta, double*map, int collision_thresh, int x_size, int y_size, int theta_size,  unordered_map <int , state*> &OPEN_STATES_3D, vector<state*> &path_3D, bool policy=true, int weight = 1)
{
    ...
    A_star_2D(dX_2D, dY_2D, start->x, start->y, map, 256, x_size, y_size, goal->x, goal->y, OPEN_STATES_3D_temp, path_3D_temp, true, weight = 1, false, false);
    ...
    while (!OPEN.empty())
    {
        ...
        current = OPEN.top();
        if (policy == false && current->x == Gx && current->y == Gy && current->theta == Gtheta) break;
        ...
        for(int dir = 0; dir < NUMOFDIRS_3D; dir++)
        {
            ...
            if (successor->x >= 0 && successor->x < x_size && successor->y >= 0 && successor->y < y_size && successor->theta >=0 && successor->theta < 360 && Is_valid(map,successor,4,4,x_size,y_size))
            {
                ...
                int cost = 0;
                if (abs(dX[dir]) + abs(dY[dir]) == 2) cost = (int)map[index_succ_2D];
                else cost = sqrt(2)*(int)map[index_succ_2D];
...
int main()
{
    ...
    // pair <array<int,NUMOFDIRS_3D>,array<int,NUMOFDIRS_3D>> delta = get_dx_dy(315);
    ...
    pair<int, int> map_size = get_dimensions("1.txt");
    double* map = create_map("1.txt",map_size);
    int dX[NUMOFDIRS] = {-1, -1, -1,  0,  0,  1, 1, 1};
    int dY[NUMOFDIRS] = {-1,  0,  1, -1,  1, -1, 0, 1};
    
    A_star_2D(dX, dY, 0, 0, map, 257, map_size.first, map_size.second, 382, 382, OPEN_STATES, path, true, 1, true, false);
    ...
    // ofstream myfile_ ("example_map.txt");
    // if (myfile_.is_open())
    // {
    ...
    int dTheta_3D[NUMOFDIRS_3D] = {315, 0, 45};
    unordered_map <int , state*> OPEN_STATES_3D;
    vector<state*> path_3D;

    A_star_3D(dTheta_3D, 250,250, 0, 125, 125, 45, map, 100, map_size.first, map_size.second, 8,  OPEN_STATES_3D, path_3D, false, 1);
    cout<<path_3D.size()<<endl;

    ofstream myfile ("example_3d.txt");
    if (myfile.is_open())
    {
